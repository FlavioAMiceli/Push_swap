...
static	int	*hoare_partition(int *arr, int low, int high)
{
	...
	while (TRUE)
	{
		while (f(arr[i], pivot) == TRUE)
			i++;
		while (f(arr[j], pivot) == FALSE)
			j--;
		if (i >= j)
			return (j);
		ft_swap(&arr[i], &arr[j]);
...
static	int	*qs(int	*arr, int low, int high, int(*f)(int, int))
{
	int	pivot_index;

	if (high - low < 10)
		&arr[low] = ft_insertion_sort(&arr[low], (high - low), f);
	else if (low < high)
	{
...
int *ft_quick_sort(int *arr, size_t len, int(*f)(int, int))
{
	return (qs(arr, 0, len - 1, f));
}
